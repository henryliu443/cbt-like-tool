...
final class MoodInsightsViewModel {
    ...
    func points(for range: Int) -> [DailyMoodPoint] {
        ...
        var grouped: [Date: [MoodCheckIn]] = [:]
        for checkin in filtered {
            let day = Calendar.current.startOfDay(for: checkin.createdAt)
            grouped[day, default: []].append(checkin)
        }
    ...
    func analysisCounts(for range: Int) -> [(Date, Int)] {
        let cutoff = Calendar.current.date(byAdding: .day, value: -range, to: Date()) ?? .distantPast
        let filtered = historyEntries.filter { $0.createdAt >= cutoff }
    ...
    func moodBreakdown(for range: Int) -> [(label: String, emoji: String, count: Int, pct: Double)] {
        ...
        var grouped: [String: [MoodCheckIn]] = [:]
        for checkin in filtered {
            grouped[checkin.moodLabel, default: []].append(checkin)
        }
        return grouped
